...
#### Scenario: Export file write is atomic
- **WHEN** the app writes a JSON or Markdown export file
- **THEN** it writes to a `.tmp` sibling file first, calls `fsync`, then calls `os.replace()` to move it to the final path, so a crash or kill mid-write never produces a truncated output file
